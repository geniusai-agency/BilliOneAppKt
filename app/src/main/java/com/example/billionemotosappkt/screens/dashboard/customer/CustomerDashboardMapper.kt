...
private fun parcelasFromContracts(contracts: List<ContratoResponse>): List<CustomerPayment> {
	return contracts
		...
		.sortedByDescending { parseApiDate(it.dueDate) ?: Date(0L) }
}
...
private fun ContratoResponse.toCustomerContract(): CustomerContract {
	...
	return CustomerContract(
		...
		contractCode = "CTR-${id.takeLast(6).uppercase(Locale.getDefault())}",
		...
		imageRes = moto?.imageRes() ?: R.drawable.moto_premium,
	)
}
...
private fun MotoResponse.imageRes(): Int {
	val candidate = listOfNotNull(modelo, marca, observacoes).joinToString(" ").lowercase()
	return when {
		candidate.contains("eletr") -> R.drawable.moto_eletrica_new
		candidate.contains("sport") -> R.drawable.moto_sport_updated
		candidate.contains("premium") -> R.drawable.moto_premium
		candidate.contains("avelloz") -> R.drawable.avelloz_160_black_new
		candidate.contains("detail") -> R.drawable.moto_detail_engine
		else -> R.drawable.moto_premium
	}
}
...
private fun formatApiDate(value: String?): String {
	...
	return runCatching {
		val parser = SimpleDateFormat("yyyy-MM-dd", Locale.forLanguageTag("pt-BR"))
		val formatter = SimpleDateFormat("dd/MM/yyyy", Locale.forLanguageTag("pt-BR"))
		val parsed = parser.parse(value.take(10)) ?: return@runCatching value
...
private fun formatShortDate(value: String?): String? {
	...
	return runCatching {
		val parser = SimpleDateFormat("yyyy-MM-dd", Locale.forLanguageTag("pt-BR"))
		val formatter = SimpleDateFormat("dd MMM", Locale.forLanguageTag("pt-BR"))
		val parsed = parser.parse(value.take(10)) ?: return@runCatching null
		formatter.format(parsed)
	}.getOrNull()
}
...
private fun parseApiDate(value: String?): Date? {
	if (value.isNullOrBlank()) return null
	return runCatching {
		SimpleDateFormat("yyyy-MM-dd", Locale.forLanguageTag("pt-BR")).parse(value.take(10))
	}.getOrNull()
}
