...
internal fun formatCompactMoneyBRL(value: Double): String = when {
    Math.abs(value) >= 1_000_000 -> "R$ ${trim1(value / 1_000_000)}M"
    Math.abs(value) >= 1_000 -> "R$ ${trim1(value / 1_000)}K"
...
private fun contratoYearMonth(contrato: ContratoResponse): String? {
    ...
    return source.takeIf { it.length >= 7 }?.substring(0, 7)
}

/**
 * Monta a visão agregada. `yearMonthNow`/`yearMonthPrev` no formato "yyyy-MM"
 * são injetados pela tela (via java.time) para manter esta função pura.
 */
